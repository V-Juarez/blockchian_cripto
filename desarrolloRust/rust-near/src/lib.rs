// Contador incremetnar, decrementar, obtener el estado, restar a 0
//
...
impl Contador {
    ...
    pub fn incrementar(&mut self) {
        self.valor += 1;
        let log_message = format!("Incremento de contador a {}", self.valor);
    ...
    pub fn decrementar(&mut self) {
        self.valor -= 1;
        let log_message = format!("Decrementar el contador a {}", self.valor);
